...
class TestAVAPFlow(AsyncHTTPTestCase):
    ...
    def get_app(self):
        # Aseguramos que el executor existe para la App
        if not TestAVAPFlow._executor:
            TestAVAPFlow._executor = AVAPExecutor(None)
            
        return Application([
            # Nota: Asegúrate de que ExecuteHandler reciba 'executor' en su initialize
    ...
    def setUp(self):
        ...
        # Singleton para el pool usando el loop de Tornado actual
        if TestAVAPFlow._pool is None:
            # Usamos el loop de esta instancia de test para crear el pool
    ...
    def tearDownClass(cls):
        # Para cerrar el pool sin conflictos de loops, usamos un nuevo loop 
        # solo si el anterior ya no es accesible
        if cls._pool:
